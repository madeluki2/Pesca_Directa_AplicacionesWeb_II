...
func construirEntornoPedido(t *testing.T) (http.Handler, string) {
	...
	token := obtenerTokenPedido(t, authSvc)
	return r, token
}

func obtenerTokenPedido(t *testing.T, authSvc *service.AuthService) string {
	t.Helper()
	_, err := authSvc.Registrar("ilaria@pedidos.com", "pedidos123")
	require.NoError(t, err)

	token, err := authSvc.Login("ilaria@pedidos.com", "pedidos123")
	require.NoError(t, err)
	return token
}
